...
**⚠️ 무료 PostgreSQL 만료 주의**: Render 무료 PostgreSQL은 생성 후 **30일 뒤 만료**되고,
**14일 유예기간** 후 업그레이드하지 않으면 데이터베이스와 데이터가 완전히 삭제됩니다
(약 생성일+44일). 웹 서비스 자체(코드)는 안 사라지지만 DB가 삭제되면 냉장고 재료/프로필
데이터가 초기화됩니다. 데모를 계속 유지하려면 만료 전에 DB를 다시 만들거나 유료 플랜으로
옮겨야 합니다 — Render가 만료 전에 이메일로 알려줍니다.
